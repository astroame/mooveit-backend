...
export const resendVerificationEmail = asyncHandler(async (req, res, next) => {
  ...
  if (!admin) return next(new ErrorResponse("There is no user with that email.", 404));

  if (admin.isVerified) return next(new ErrorResponse("Email already verified.", 400));

  // Get reset token
  const verificationToken = await admin.getVerificationToken(admin);
  ...
  >
    ...
    >
      ...
      <p style="text-align: left; margin-top: 15px; color: rgb(83, 83, 83)">
       Hi ${admin.firstName}, please kindly click on the link to verify your email address. Note that the link would expire after 24 hours. 
        <br />
...
export const approveListing = asyncHandler(async (req, res, next) => {
  ...
  if (!storageListing) return next(new ErrorResponse("There is no storage with that ID", 404));
